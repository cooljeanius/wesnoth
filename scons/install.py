...
def InstallBinary(env, source):
    ...
    installdir = env.subst(os.path.join(
...
def InstallData(env, datadir, component, source, subdir="", **kwargs):
    installdir = Dir(env.subst(os.path.join(
        env["destdir"], env[datadir].lstrip("/"), subdir)))
    ...
    for source in sources:
        if isinstance(source, SCons.Node.FS.Dir) or source.isdir():
            ...
            env.Alias("install-" + component,
                      env.Install(installdir, source, **kwargs))
    if dirs:
        if len(dirs) == 1:
            install = env.InstallFiltered(
                installdir.path, dirs[0].path, **kwargs)
        else:
            install = [env.InstallFiltered(os.path.join(
                installdir.path, x.name, **kwargs), x.path) for x in dirs]
        AlwaysBuild(install)
...
def generate(env):
    ...
    env.Append(BUILDERS={'InstallFiltered': Builder(
        action=InstallFilteredHook, target_factory=Dir, source_factory=Dir)})
