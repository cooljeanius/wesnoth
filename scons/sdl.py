...
def CheckSDL2(context, require_version):
    ...
    if env["PLATFORM"] != "win32" or sys.platform == "msys":
        ...
        if sdldir:
            env.AppendUnique(
                CPPPATH=[os.path.join(sdldir, "include/SDL2")],
    ...
    with open(cpp_file, "r") as file:
        test_program = (
            file.read()
            .replace("argv[1]", '"' + major_version + '"')
            .replace("argv[2]", '"' + minor_version + '"')
            .replace("argv[3]", '"' + patchlevel + '"')
        )
...
def CheckWebP(context):
    ...
    img_file1 = (
        File("data/core/images/scons_conftest_images/end-n.webp").rfile().abspath
    )
