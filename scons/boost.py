...
def find_boost(env):
    ...
    includes = find_include(prefixes, "boost/config.hpp",
    ...
    if includes:
        ...
        for prefix, includefile in includes:
            try:
                ...
                versions.append((0, 0))
        version_nums = [100000 * major_minor[0] + 100 *
                        major_minor[1] for major_minor in versions]
...
def CheckBoost(context, boost_lib, require_version=None, header_only=False):
    ...
    if sys.platform == "win32" or normpath(boostdir) != "/usr/include":
        if env["fast"]:
            env.AppendUnique(
                CXXFLAGS=["-isystem", boostdir], LIBPATH=[boostlibdir])
        else:
...
config_checks = {"CheckBoost": CheckBoost, "CheckBoostIostreamsGZip": CheckBoostIostreamsGZip,
                 "CheckBoostIostreamsBZip2": CheckBoostIostreamsBZip2, "CheckBoostLocaleBackends": CheckBoostLocaleBackends}
