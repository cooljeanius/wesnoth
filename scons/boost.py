...
def find_boost(env):
    ...
    include = find_include(
        prefixes, "boost/config.hpp", default_prefixes=not crosscompile
    )
    if include:
    ...
    includes = find_include(
        prefixes, "boost/config.hpp", include_subdir="include/boost-*"
    )
    if includes:
        versions = []
            ...
            for lib in libs:
                try:
                    env["boost_suffix"] = re.findall(
                        r"libboost_\w*(-.*%d_%d)" % tuple(version), lib
                    )[0]
                    break
                except:
                    pass
...
def CheckBoost(context, boost_lib, require_version=None, header_only=False):
    ...
    if require_version:
        context.Message(
            "Checking for Boost %s library version >= %s... "
...
config_checks = {
    ...
    "CheckBoostIostreamsGZip": CheckBoostIostreamsGZip,
    "CheckBoostIostreamsBZip2": CheckBoostIostreamsBZip2,
    "CheckBoostLocaleBackends": CheckBoostLocaleBackends,
}
