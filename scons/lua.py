...
def CheckLibsWithSystemHeader(context, libs, header, language):
    res = SCons.Conftest.CheckLib(
        context, libs, header="#include <%s>" % header, language=language
    )
...
def CheckLua(context, require_version):
    ...
    for include_subdir_raw in include_subdirs_raw:
        include_subdirs[len(include_subdirs) :] = [
            include_subdir_raw,
    ...
    if luadir:
        for include_subdir in include_subdirs:
            includes = find_include(
                [luadir], "lua.h", include_subdir=include_subdir, default_prefixes=False
            )
