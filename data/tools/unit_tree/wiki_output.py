...
def main():
    wesnoth = helpers.WesnothList(
        options.wesnoth, options.config_dir, options.data_dir, options.transdir
    )
    ...
    for campaign in campaigns:
        ...
        if define is not None:
            campaign_defines.append(define)
        if ed is not None:
            campaign_defines.extend(ed.split(","))
    ...
    for campaign, unitslists in list(punits.items()):
        for unitlist in unitslists:
            for race in unitlist.get_all(tag="race"):
                ...
                if race_id is None:
                    race_id = race.get_text_val("name")
                if race_id is None:
                    continue
                else:
                    races[race_id] = race
    ...
    for campaign, unitslists in list(punits.items()):
        for unitlist in unitslists:
            for unit in unitlist.get_all(tag="unit_type"):
                if unit.get_text_val("do_not_list") in ["yes", "true"]:
                    continue
                if unit.get_text_val("hide_help") in ["yes", "true"]:
                    continue
    ...
    for unit in list(all_units.values()):
        ...
        try:
            unit.level = int(base_val(unit, "level"))
        except TypeError:
            unit.level = 0

        r = base_val(unit, "race")
        try:
            unit.race = races[r].get_text_val(
                "plural_name", translation=translated.translate
            )
        except KeyError:
            unit.race = "-"
        ...
        if not a or a == "null":
            ...
            unit.advances_to = [x.strip() for x in a.split(",")]
    ...
    def race_key(unit):
        if unit.campaign == "mainline":
            return 0, unit.race
        else:
            return 1, unit.campaign

    # Group by race/campaign
    units_per_race = {}
    for unit in list(all_units.values()):
        x = race_key(unit)
        if x not in units_per_race:
            units_per_race[x] = set()
