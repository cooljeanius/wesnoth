    ...
    if eras:
        ...
        for era in eras:
            ...
            w(
                ...
                + epath
                + '">'
                + html_output.cleantext(era["name"], quote=False)
                + "</a></li>"
            )
        w("</ul>")
...
def main(folder):
    ...
    w(
        html_output.website_header(
            path="", title="Database Build Report", classes=["wmlunits-report"]
        )
    )
    ...
    for f in sorted(glob.glob(os.path.join(folder, "*"))):
        ...
        w(
            '<a href="'
            + html_output.cleanurl(os.path.join(name, "index.html"))
            + '">'
            + html_output.cleantext(name, quote=False)
            + "</a>"
        )
        w("</td><td>")
        w(str(n))
        ...
        if os.path.exists(error_log):
            ...
            def postprocess(line):
                if (
                    line
                    in ("WMLError:", "?", "Preprocessor error:", "'SKIP_CORE' defined.")
                    or line.startswith("Automatically found a possible data directory")
                    or line.startswith("Overriding data directory with")
                    or line.startswith("skipped 'data/core'")
                    or line.startswith("preprocessing specified resource:")
                    or re.match("added .* defines.", line)
                ):
                    return ""
            ...
            for line in text.splitlines():
                ...
                if line in [
                    ...
                    htmlerr.write(
                        '<p class="error %s">' % line[1:-1].replace(" ", "-").lower()
                    )
                elif line in [
                    "</INTERNAL ERROR>",
                    "</WML ERROR>",
            ...
            total_error_logs += 1
            w(
                '<a class="error" href="%s">%s (%d lines)</a>'
