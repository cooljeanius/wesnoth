...
class Addon(object):
    ...
    def __init__(self, github, name, readonly):
        ...
        """
        logging.debug(
            "Addon created with name {0} and version {1}{2}".format(
    ...
    def update(self):
        ...
        if "Already up-to-date." in out:
            ...
            if self.readonly:
                logging.error(
                    "Untracked files blocking pull of {0}. Attempting to remove...".format(
                        self.name
                    )
                )
                remove_untracked()
                ...
                logging.error(
                    "Untracked files blocking pull of {0}. Please remove.".format(
                        self.name
                    )
                )
            return False
            ...
            logging.info(
                "Pulled from still-empty (not initialized) repository {0}.".format(
                    self.name
                )
            )
            return False
    ...
    def sync_from(self, src, exclude):
        ...
        self._copytree(
            src,
            self.get_dir(),
            ignore=lambda src, names: [n for n in names if n in exclude],
        )
    ...
    def commit(self, message):
        ...
        if not statusline:
            ...
            if status == " ":
                ...
                # Up to date?
                logging.warn(
                    "Commit to add-on {0} with message '{1}' has not made any changes".format(
                        self.name, message
                    )
                )
            elif status == "!":
                raise AddonError(
                    self.name,
                    "Commit with message '{0}' failed for reason {1}".format(
                        message, summary
                    ),
                )
            else:
                raise AddonError(
                    self.name,
                    "Commit with message '{0}' has done something unexpected: {1}".format(
    ...
    def _rmtree(self, directory, exclude):
        logging.debug(
            "Deleting tree {0}, except for {1}".format(self.name, ",".join(exclude))
        )
    ...
    def _execute(self, command, check_error=False):
        return self.github._execute(
            command, cwd=self.get_dir(), check_error=check_error
        )
...
class GitHub(object):
    ...
    def __init__(self, directory, version, authorization=None):
        ...
        """
        logging.debug(
            "GitHub created with directory {0} and version {1}, {2} authentication data".format(
    ...
    def _github_repos_list(self, readonly=False):
        ...
        return [
            (
                repo["name"][: -len(version_suffix)],
                repo["git_url"] if readonly else repo["ssh_url"],
            )
            for repo in self._github_repos_memo
            if repo["name"].endswith(version_suffix)
        ]
    ...
    def _github_repos_create(self, name):
        ...
        requestdata = {
            ...
            "events": ["push"],
            "active": True,
            "config": {"url": "http://ai0867.net:6660/wescamp", "content_type": "json"},
        }
    ...
    def _github_api_request(self, url, data=None, method=None, authenticate=False):
        ...
        if link_headers:
            ...
            links_split_proc = [
                (
                    l[1].strip().lstrip('rel="').rstrip('"'),
                    l[0].strip().lstrip("<").rstrip(">"),
                )
                for l in links_split_raw
            ]
    ...
    def _execute(self, command, cwd=None, check_error=False):
        ...
        if check_error and len(err):
            raise Error(
                "Failure executing command '{0}': {1}".format(" ".join(command), err)
            )
...
def _gen(possible_dirs):
    def _get_build_system(possible_dirs):
        ...
        if not isinstance(possible_dirs, list):
            raise Error(
                "Incorrect argument type passed, {0} instead of {1}".format(
