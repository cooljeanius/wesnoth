...
if __name__ == "__main__":
    ...
    def update_build_system(addon_obj, addon_name, wescamp_dir, build_sys_dir):
        ...
        """
        logging.info(
            "Checking if build system for add-on {0} needs to be updated".format(addon_name))
        ...
        if len(build_system_version) != 40:
            logging.warn("Incorrect SHA1 for build system checkout: {0}".format(
        ...
        out, err, res = addon_obj._execute(
            [init_script, "--{0}".format(git_version), addon_name, "."], check_error=False)
        if len(err):
            logging.warn(
        ...
        addon_obj._execute(["git", "add", "po", "campaign.def",
                           "Makefile", BUILDSYS_FILE], check_error=True)
        if previously_initialized:
            logging.info(
                "Updated build system for add-on {0}".format(addon_name))
            addon_obj.commit("wescamp.py: Update build-system")
        else:
            logging.info(
                "Initialized build system for add-on {0}".format(addon_name))
    ...
    def pot_update(addon_obj, addon_name):
        ...
        if not os.path.exists(os.path.join(addon_obj.get_dir(), "Makefile")):
            logging.warn(
                "Cannot pot-update: build system does not exist for add-on {0}.".format(addon_name))
            return False
        # Ugliness, again
        out, err, res = addon_obj._execute(["make"])
        if len(err):
            logging.warn(
                "pot-update in addon {0}:\n{1}".format(addon_name, err))
            # TODO: bail?

        if res != 0:
            logging.error(
                "Failed to pot-update for add-on {0}".format(addon_name))
    ...
    def upload(server, addon, temp_dir, wescamp_dir, build_sys_dir=None):
        ...
        github = libgithub.GitHub(
            wescamp_dir, git_version, authorization=git_auth)
    ...
    def checkout(wescamp, auth=None, readonly=False):
        ...
        logging.debug(
            "checking out add-ons from wesnoth version = '%s' to directory '%s'", git_version, wescamp)
    ...
    # Upload all addons from wescamp.
        ...
        for k, v in list(addons.items()):
            try:
                ...
                print("[ERROR github in {0}] {1}".format(
                    e.addon, str(e.message)))
                error = True
            except libgithub.Error as e:
