...
class TmxFileFormat(FileFormatHandler):
    ...
    def write(self, file_or_filename, journey, metadata=None):
        ...
        ET.SubElement(tileset, "grid", attrib={
        ...
        # background image
        layer = ET.SubElement(tmxmap, "imagelayer", attrib={
        ...
        for track in journey.tracks:
            name = track.name
            layer = ET.SubElement(tmxmap, "objectgroup", attrib={
                                  "id": id_counter.get(), "name": name})
        ...
        ET.SubElement(custom_properties, "property", attrib={
                      "name": "tmx_trackplacer_export_id", "type": "int", "value": id_counter.get()})
    ...
    def read(self, fp):
        ...
        if len(tmxmap.findall("imagelayer/image")) != 1:
            raise ParseException(
                "expected exactly one image in the imagelayer")
