...
class PoCommentedString:
    ...
    def update_with_commented_string(self, commented_string):
        ...
        # a plural form
        if self.plural is None and isinstance(
            commented_string.plural, PoCommentedStringPL
        ):
...
class WmlNode:
        ...
        *,
        ...
        if plural is not None:
            if isinstance(plural, str):
                ...
                if len(plural) == 2:
                    if isinstance(plural[0], str) and isinstance(plural[1], bool):
                        plural_value = PoCommentedStringPL(plural[0], plural[1])
                    elif isinstance(plural[0], bool) and isinstance(plural[1], str):
                        plural_value = PoCommentedStringPL(plural[1], plural[0])
    ...
    def assemble_orderid(self, nodesentence):
        return (self.fileno, nodesentence.lineno, nodesentence.lineno_sub)
            ...
            if nodesentence.overrideinfo == "":
                if nodesentence.addedinfo is not None and nodesentence.addedinfo != "":
                    return PoCommentedString(
                        ...
                        wmlinfos=[],
                        finfos=[self.fileref + str(nodesentence.lineno)],
