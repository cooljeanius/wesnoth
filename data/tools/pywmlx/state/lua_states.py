...
class LuaCheckdomState:
    def __init__(self):
        rx = (
            r"\s*(local\s+)?_\s*=\s*wesnoth\s*\.\s*textdomain\s*"
            r"""(?:\(\s*)?(["'])(.*?)\2"""
        )
...
class LuaCheckpoState:
    def __init__(self):
        self.regex = re.compile(r"\s*--\s*(?:#)?\s*(po-override|po):\s+(.+)", re.I)
...
class LuaStr00:
    ...
    def run(self, xline, lineno, match):
        ...
        pywmlx.state.machine._pending_luastring = None
        pywmlx.state.machine._pending_luastring = pywmlx.state.machine.PendingLuaString(
            lineno,
            "lua_plural",
            "",
            False,
            True,
            0,
            pywmlx.state.machine.PendingPlural(),
        )
...
class LuaStr01:
    ...
    def run(self, xline, lineno, match):
        ...
        if match.group(3) == "":
            ...
            xline = xline[match.end() :]
        pywmlx.state.machine._pending_luastring = pywmlx.state.machine.PendingLuaString(
            lineno, "luastr1", match.group(2), loc_multiline, loc_translatable
        )
        return (xline, _nextstate)
...
class LuaStr02:
    ...
    def run(self, xline, lineno, match):
        ...
        if match.group(3) == "":
            ...
            xline = xline[match.end() :]
        pywmlx.state.machine._pending_luastring = pywmlx.state.machine.PendingLuaString(
            lineno, "luastr2", match.group(2), loc_multiline, loc_translatable
        )
        return (xline, _nextstate)
...
class LuaStr03:
    ...
    def run(self, xline, lineno, match):
        ...
        loc_multiline = False
        pywmlx.state.machine._pending_luastring = pywmlx.state.machine.PendingLuaString(
            lineno, "luastr3", match.group(3), loc_multiline, loc_translatable
        )
        return (xline, "lua_idle")
...
class LuaStr30:
    ...
    def run(self, xline, lineno, match):
        rx = r"(.*?)]={" + str(pywmlx.state.machine._pending_luastring.numequals) + "}]"
        realmatch = re.match(rx, xline)
        _nextstate = "lua_str30"
        if realmatch:
            pywmlx.state.machine._pending_luastring.addline(realmatch.group(1))
    ...
    def run(self, xline, lineno, match):
        ...
        if status == "wait_string":
            ...
            if match.group(2) == "":
                ...
                xline = xline[match.end() :]
                pywmlx.state.machine._pending_luastring.plural.status = "wait_plural"
            ...
            if match.group(2) == "":
                ...
                xline = xline[match.end() :]
                pywmlx.state.machine._pending_luastring.plural.status = "wait_close"
            # write first line of 'plural' on PendingLuaString
            pywmlx.state.machine._pending_luastring.plural.string = match.group(1)
            return (xline, _nextstate)
...
class LuaPl03:
    ...
    def run(self, xline, lineno, match):
        ...
        if status == "wait_string":
            ...
            pywmlx.state.machine._pending_luastring.luastring = match.group(2)
            pywmlx.state.machine._pending_luastring.plural.status = "wait_plural"
        else:
            _nextstate = "lua_plidle3"
            # we must fix values for plural.pluraltype
            ...
            pywmlx.state.machine._pending_luastring.plural.string = match.group(2)
            pywmlx.state.machine._pending_luastring.plural.status = "wait_close"
        return (xline, _nextstate)
...
class LuaPl03o:
    ...
    def run(self, xline, lineno, match):
        ...
        if status == "wait_string":
            ...
            pywmlx.state.machine._pending_luastring.luatype = "luastr3"
            pywmlx.state.machine._pending_luastring.luastring = match.group(2)
            pywmlx.state.machine._pending_luastring.numequals = len(match.group(1))
        else:
            pywmlx.state.machine._pending_luastring.plural.ismultiline = True
            pywmlx.state.machine._pending_luastring.plural.pluraltype = 3
            pywmlx.state.machine._pending_luastring.plural.string = match.group(2)
            pywmlx.state.machine._pending_luastring.plural.numequals = len(
                match.group(1)
            )
...
class LuaPl10:
    ...
    def run(self, xline, lineno, match):
        ...
        if status == "wait_string":
            ...
            pywmlx.state.machine_pending_luastring.plural.addline(match.group(1))
        if match.group(2) == '"':
            xline = xline[match.end() :]
            if status == "wait_string":
                _nextstate = "lua_plidle2"
                pywmlx.state.machine_pending_luastring.plural.status = "wait_plural"
            else:
                _nextstate = "lua_plidle3"
                pywmlx.state.machine_pending_luastring.plural.status = "wait_close"
        else:
            xline = None
        return (xline, _nextstate)
...
class LuaFinalState:
    ...
    def run(self, xline, lineno, match):
        rx_str = (
            r"function\s+([a-zA-Z0-9_.]+)|" + r"([a-zA-Z0-9_.]+)\s*=\s*function"  # +
        )  # r'(local)\s+(?!function).*?=' )
...
def setup_luastates():
    for statename, stateclass in [
        ("lua_idle", LuaIdleState),
        ...
        st = stateclass()
        pywmlx.state.machine.addstate(statename, State(st.regex, st.run, st.iffail))
