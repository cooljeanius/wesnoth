...
class WmlStr02:
    ...
    def run(self, xline, lineno, match):
        ...
        # (group(1) will not exist, than)
            ...
            if mydebug:
                err_message = (
                    "wmlstr02 assertion error (DEBUGGING)\n"
                    + "g1: "
                    + str(match.group(1))
                    + "\n"
                    + "g2: "
                ...
                wmlerr(
                    "wmlxgettext python sources",
                    "wmlstr02 assertion error\n"
                    "please report a bug if you encounter this error message",
                )
        pywmlx.state.machine._pending_wmlstring = pywmlx.state.machine.PendingWmlString(
...
class WmlTagState:
    ...
    def run(self, xline, lineno, match):
        ...
        if match.group(1) == "/":
            ...
            pywmlx.nodemanip.closenode(
                closetag, pywmlx.state.machine._dictionary, lineno
            )
            if closetag == "[/lua]":
...
class WmlGetinfState:
    def __init__(self):
        rx = r"\s*(speaker|id|role|description|condition|type|race)" + r"\s*=\s*(.*)"
        self.regex = re.compile(rx, re.I)
...
class WmlStr20:
    ...
    def run(self, xline, lineno, match):
        ...
        _nextstate = "wml_str20"
        if realmatch:
            pywmlx.state.machine._pending_wmlstring.addline(realmatch.group(1))
