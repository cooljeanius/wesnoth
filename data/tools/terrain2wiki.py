...
terrain_url = (
    "https://raw.github.com/wesnoth/wesnoth/1.14/data/core/images/terrain/%s.png"
)
...
def parse_terrain(data):
    ...
    terrains = re.compile(
        r"\[terrain_type\](.*?)\[\/terrain_type\]", re.DOTALL
    ).findall(data)
...
""" % (
            terrain_url
            % (
                content["editor_image"]
                if "editor_image" in content
                else content["symbol_image"]
            ),
            content["editor_name"][4:-1]
            if "editor_name" in content
            else content["name"][4:-1],
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="terrain2wiki is a tool to\
convert the terrain codes located in terrain.cfg to wiki formatted text."
    )
    parser.add_argument(
        "-f",
        "--file",
        default="data/core/terrain.cfg",
        dest="path",
        help="The location of the terrain.cfg file.",
    )
    parser.add_argument(
        "-o",
        "--output",
        default="/tmp/TerrainCodeTableWML",
        dest="output_path",
