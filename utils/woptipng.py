...
parser.add_argument(
    "inpath",
    help="file or path (recursively) to be searched for crushable pngs",
    metavar="path",
    nargs="+",
    type=str,
)
...
parser.add_argument(
    "-t",
    "--threshold",
    help="size reduction below this percentage will be discarded, default: 10",
    metavar="n",
    nargs="?",
    default=10,
    type=float,
...
parser.add_argument(
    "-j",
    "--jobs",
    help="max amount of jobs/threads. If unspecified, take number of cores found",
    metavar="n",
    nargs="?",
...
parser.add_argument(
    "-n",
    "--nice",
    help="niceness of all threads (must be positive, \
doesn't have any effect on Windows)",
    metavar="n",
    nargs="?",
    default=19,
    type=int,
...
def run_imagemagick(image, tmpimage):
    ...
    cmd = [EXEC_IMAGEMAGICK, "-strip", "-define", "png:color-type=6", image, tmpimage]
    subprocess.call(cmd)
...
def run_optipng(image, tmpimage):
    debugprint("optipng...")
    shutil.copy(image, tmpimage)
    cmd = [EXEC_OPTIPNG, "-q", "-o5", "-nb", "-nc", "-np", tmpimage]
    subprocess.call(cmd)
...
if files_optimized:
    print(
        "{files_optimized} of {files_processed} files optimized, {size_before} bytes reduced to {size_after} bytes; {size_diff} bytes, {percentage_delta}%".format(
            files_optimized=files_optimized,
            files_processed=len(result_list),
            size_before=size_before,
            size_after=size_after,
            size_diff=size_after - size_before,
            percentage_delta=str((size_after - size_before) / (size_before) * 100)[0:6],
        )
    )
else:
...
if threshold_hit:
    print(
        "The following files could be reduced, but didn't meet the optimization threshold ({threshold}%), the --threshold option controls this".format(
    ...
    for i in result_list:
        if i.status == ProcessingStatus.REVERTED_THRESHOLD:
            print(
                "{percentage_delta}% {name}, {size_initial} bytes reduced to {size_after} bytes; {size_diff} bytes".format(
                    name=i.name,
                    size_initial=i.size_initial,
                    size_after=i.size_after,
                    size_diff=i.size_after - i.size_initial,
                    percentage_delta=str(
                        (i.size_after - i.size_initial) / (i.size_initial) * 100
                    )[0:6],
                )
            )
