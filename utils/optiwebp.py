...
args.add_argument("--quality", default="90",
                  help="The quality to use for lossy compression. Defaults to 90.")
args.add_argument("--repo", default=os.getcwd(),
                  help="The root of the Wesnoth repository. Defaults to the current directory.")
args.add_argument("--tempdir", default=os.path.join(os.getcwd(), "temp_webp"),
...
image_dirs = [
    ...
    os.path.join("data", "campaigns", "Delfadors_Memoirs",
                 "images", "portraits"),
    os.path.join("data", "campaigns",
                 "Descent_Into_Darkness", "images", "maps"),
    os.path.join("data", "campaigns", "Descent_Into_Darkness",
                 "images", "portraits"),
    os.path.join("data", "campaigns",
                 "Descent_Into_Darkness", "images", "story"),
    os.path.join("data", "campaigns", "Eastern_Invasion",
                 "images", "portraits"),
    os.path.join("data", "campaigns", "Eastern_Invasion", "images", "maps"),
    os.path.join("data", "campaigns", "Heir_To_The_Throne",
                 "images", "portraits"),
    ...
    os.path.join("data", "campaigns", "Legend_of_Wesmere",
                 "images", "portraits"),
    ...
    os.path.join("data", "campaigns", "Northern_Rebirth",
                 "images", "portraits"),
    os.path.join("data", "campaigns", "Sceptre_of_Fire", "images", "maps"),
    os.path.join("data", "campaigns", "Sceptre_of_Fire",
                 "images", "portraits"),
    os.path.join("data", "campaigns",
                 "Secrets_of_the_Ancients", "images", "story"),
    os.path.join("data", "campaigns", "Secrets_of_the_Ancients",
                 "images", "portraits"),
    os.path.join("data", "campaigns",
                 "Son_Of_The_Black_Eye", "images", "maps"),
    os.path.join("data", "campaigns", "Son_Of_The_Black_Eye",
                 "images", "portraits"),
    os.path.join("data", "campaigns",
                 "The_Hammer_of_Thursagan", "images", "maps"),
    os.path.join("data", "campaigns", "The_Hammer_of_Thursagan",
                 "images", "portraits"),
    os.path.join("data", "campaigns", "The_Rise_Of_Wesnoth", "images", "maps"),
    os.path.join("data", "campaigns", "The_Rise_Of_Wesnoth",
                 "images", "portraits"),
    os.path.join("data", "campaigns",
                 "The_Rise_Of_Wesnoth", "images", "story"),
    os.path.join("data", "campaigns", "The_South_Guard",
                 "images", "portraits"),
    ...
    os.path.join("data", "campaigns", "Under_the_Burning_Suns",
                 "images", "portraits"),
...
results_dict = {}
lossy_args = ["cwebp", "-mt", "-m", "6",
              "-q", options.quality, "-alpha_q", "100"]
    ...
    for image_root, _, files in os.walk(os.path.join(options.repo, image_dir)):
        for filename in files:
            ...
            if filetype == ".png":
                ...
                with open(os.path.join(image_root, filename), "rb") as img:
                    ...
                    if webp_type == b"VP8L" or webp_type == b"VP8X":
                        ...
                        print(filename, "is not a valid WebP file",
                              file=sys.stderr)
                        continue
            ...
            result = subprocess.run(
                subprocess_args + [initial_file, "-o", webp_file], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    ...
    with open("conversion-good.tsv", "w") as good_f, open("conversion-bad.tsv", "w") as bad_f:
        ...
        for filename, data in results_dict.items():
            line = "{filename}\t{old_size}\t{new_size}\t{change_in_percent}\t{change_in_bytes}\n".format(
                filename=filename, old_size=data["old_size"], new_size=data["new_size"], change_in_percent=data["change_in_percent"], change_in_bytes=data["change_in_bytes"])
...
print("Total size for converted images changed from {initial} to {final} ({percentage}%)".format(
    initial=initial_total_size, final=final_total_size, percentage=total_percentage_change))
print("Took {hours:02d}:{minutes:02d}:{seconds:02d} to process {count} files.".format(
    hours=hours_duration, minutes=minutes_duration, seconds=seconds_duration, count=len(results_dict)))
