...
class BoostUnorderedMapPrinter(object):
    ...
    class _iterator:
        def __init__(self, val):
            ...
            if self.buckets != 0:
                ...
                self.base_pointer = gdb.lookup_type(
                    "boost::unordered_detail::value_base< %s >" % pair
                ).pointer()
                self.node_pointer = gdb.lookup_type(
                    "boost::unordered_detail::hash_node<std::allocator< %s >, boost::unordered_detail::ungrouped>"
                    % pair
                ).pointer()
        ...
        def __next__(self):
            ...
            iterator = (
                self.node.cast(self.node_pointer)
                .cast(self.base_pointer)
                .cast(self.pair_pointer)
                .dereference()
            )
...
class BoostUnorderedMapIteratorPrinter(object):
    def __init__(self, val):
        ...
        self.bucket_pointer = gdb.lookup_type(
            "boost::unordered_detail::hash_bucket<std::allocator< %s > >" % pair
        ).pointer()
...
def add_printers(pretty_printers_dict):
    pretty_printers_dict[
        re.compile("^n_interned::t_interned_token.*$")
    ] = T_InternedPrinter
    pretty_printers_dict[re.compile("^n_token::t_token$")] = T_TokenPrinter
    pretty_printers_dict[re.compile("^t_string$")] = TstringPrinter
    pretty_printers_dict[
        re.compile("^config::attribute_value$")
    ] = AttributeValuePrinter
    pretty_printers_dict[
        re.compile("^boost::unordered_map.*$")
    ] = BoostUnorderedMapPrinter
    pretty_printers_dict[
        re.compile(
            "^boost::unordered_detail::hash_iterator\<std::allocator\<std::pair\<.*$"
        )
    ] = BoostUnorderedMapIteratorPrinter
