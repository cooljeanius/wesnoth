...
def save_result_database(cfg, test, game_result, sqlite_file):
    print("Saving to DB....")
    query = (
        'INSERT INTO games("test_id","faction1","faction2","switched_side","is_success","end_turn","winner")'
        + "VALUES (?,?,?,?,?,?,?)"
    )
...
if cfg.has_option("default", "log_file"):
    log_file = open(
        datetime.datetime.now().strftime(cfg.get("default", "log_file").strip()), "w"
    )
    log_file.write(
        '"ai_config1", "ai_config2", "ai_ident1", "ai_ident2", "switched_side", '
...
if cfg.has_option("default", "sqlite_file"):
    ...
    cur = conn.cursor()
    cur.execute(
        'INSERT INTO tests ("title","ai_config1","ai_config2","map","time") '
        + 'VALUES ("'
        + test.title
        + '","'
        + test.ai_config1
...
for game_results in executions(cfg, test):
    ...
    print(
        "AI1(" + ai1 + ") won: " + str(ai1_won) + "/" + str(ai1_won * 100 / total) + "%"
    )
    print(
        "AI2(" + ai2 + ") won: " + str(ai2_won) + "/" + str(ai2_won * 100 / total) + "%"
    )
