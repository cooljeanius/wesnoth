...
def construct_command_line(cfg, test, switched_side):
    ...
    if repeats > 1:
        print('Be patient, ' + str(repeats) +
              ' repeats are going to take a while.')
...
def run_game(cfg, test, switched_side):
    ...
    p = Popen(command_line, shell=True, bufsize=10000000,
...
def executions(cfg, test):
    ...
    if structured:
        factions = ['Drakes', 'Rebels', 'Undead',
                    'Northerners', 'Knalgan Alliance', 'Loyalists']
        i = 1
        for faction1 in factions:
            for faction2 in factions:
                print('EXECUTION: ' + str(i) + '/36 --- ' +
                      faction1 + ' against ' + faction2)
...
if cfg.has_option('default', 'log_file'):
    log_file = open(datetime.datetime.now().strftime(
        cfg.get('default', 'log_file').strip()), 'w')
...
for game_results in executions(cfg, test):
    ...
    print('AI1(' + ai1 + ') won: ' + str(ai1_won) +
          "/" + str(ai1_won * 100 / total) + '%')
    print('AI2(' + ai2 + ') won: ' + str(ai2_won) +
          "/" + str(ai2_won * 100 / total) + '%')
